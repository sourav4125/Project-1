...
const updateData = async function (req, res) {
    try{let upd = req.body
    let { title, body, tags, subcategory } = upd
    let time = moment().format()
    let data5 = req.params.blogId
    let update = await blogModel.findByIdAndUpdate({_id:data5}, { $set: { title:title,body:body,isPublished: true, publishedAt: time }, $push: { tags: tags, subcategory: subcategory } }, { new: true })
    if (update) {
...
const deleteData=async function(req,res){
    try{let data5 = req.params.blogId
    const blogDelete=await blogModel.findByIdAndUpdate({_id:data5},{$set:{isDeleted:true}},{new:true})
    if(!blogDelete){
...
const deleteData1=async function(req,res){
    try{let data6=req.query
    let time=moment().format()
    
    const blog1Delete=await blogModel.findOneAndUpdate({data6},{$set:{isDeleted:true,deletedAt:time}},{new:true})
    if(blog1Delete){
